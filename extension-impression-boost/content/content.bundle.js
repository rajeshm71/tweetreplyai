...
(() => {
  ...
  // extension-impression-boost/content/content.js
  (function initImpressionBoostAutoRefresh() {
      ...
      if (done >= target) {
        ...
        pendingTimer = setTimeout(async () => {
          ...
          try {
            ...
            console.error(LOG, "long pause error:", e?.message || e);
          }
        }, Lsec * 1e3);
        return;
      }
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      const relevant = changes[k.enabled] || changes[k.urlPrefix] || changes[k.shortDelayMinSec] || changes[k.shortDelayMaxSec] || changes[k.longPauseMinSec] || changes[k.longPauseMaxSec] || changes[k.reloadBurstMin] || changes[k.reloadBurstMax] || changes[k.readingSimEnabled] || changes[k.readingIntervalMin] || changes[k.readingIntervalMax];
      if (!relevant) return;
      clearPending();
      void run();
    });
