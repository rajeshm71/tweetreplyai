import { AUTO_REFRESH, randomIntInclusive } from '../config/constants.js';
...
class PopupManager {
  ...
  async loadSettings() {
    ...
    const result = await chrome.storage.local.get([
      k.enabled, k.urlPrefix,
      k.shortDelayMinSec, k.shortDelayMaxSec,
      k.longPauseMinSec,  k.longPauseMaxSec,
      k.reloadBurstMin,   k.reloadBurstMax,
      k.readingSimEnabled, k.readingIntervalMin, k.readingIntervalMax,
    ]);
  ...
  validateNumericFields() {
    ...
    if (shortMin < L.shortDelayMinSec || shortMin > L.shortDelayMaxSec ||
        shortMax < L.shortDelayMinSec || shortMax > L.shortDelayMaxSec) {
    ...
    if (longMin < L.longPauseMinSec || longMin > L.longPauseMaxSec ||
        longMax < L.longPauseMinSec || longMax > L.longPauseMaxSec) {
      return { ok: false, error: `Long pause: ${L.longPauseMinSec}–${L.longPauseMaxSec}s.` };
    }
    if (longMin > longMax) {
      return { ok: false, error: 'Long pause min must be ≤ max.' };
    }
    if (burstMin < L.reloadBurstMin || burstMin > L.reloadBurstMax ||
        burstMax < L.reloadBurstMin || burstMax > L.reloadBurstMax) {
    ...
    if (readingMin < L.readingIntervalMin || readingMin > L.readingIntervalMax ||
        readingMax < L.readingIntervalMin || readingMax > L.readingIntervalMax) {
    ...
    return {
      ok: true,
      shortMin, shortMax, longMin, longMax, burstMin, burstMax,
  ...
  normalizeUrlPrefix(raw) {
    ...
    let u;
    try { u = new URL(trimmed.includes('://') ? trimmed : `https://${trimmed}`); }
    ...
    // X/Twitter: strip www — browser address bar shows x.com (no www)
    if (host === 'www.x.com') u.hostname = 'x.com';
    if (host === 'www.twitter.com') u.hostname = 'twitter.com';
    // LinkedIn: ensure www is present — browser always shows www.linkedin.com
  ...
  async saveSettings() {
    ...
    if (!numeric.ok) { this.setStatus(numeric.error, true); return; }
    ...
    if (enabled) {
      ...
      if (!urlNorm.ok) { this.setStatus(urlNorm.error, true); return; }
