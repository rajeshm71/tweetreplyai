  ...
  var BackgroundManager = class {
    ...
    async handleSyncAuthFromTab(tabId, sendResponse) {
      try {
        ...
        console.error("Failed to sync auth from tab:", error2);
      ...
      chrome.commands?.onCommand?.addListener(async (command) => {
        try {
          ...
          this.enqueueTelemetry({
            ...
            surface: "background",
            error_code: error2?.message || "shortcut_dispatch_failed",
    ...
    async checkForAuthCompletion(url, tabId) {
      ...
      if (isAuthSuccess) {
        try {
          ...
          console.error("Failed to extract auth token:", error2);
          this.enqueueTelemetry({
            event_type: "auth_sync_failed",
            surface: "background",
    ...
    async requestAuthFromWebApp(tabId) {
      try {
        const results = await chrome.scripting.executeScript({
          ...
          function: async () => {
            try {
              ...
              console.error("Failed to get auth from web app:", error2);
            }
            return null;
          }
        });
        ...
        console.error("Failed to request auth from web app:", error2);
        this.enqueueTelemetry({
          event_type: "auth_sync_failed",
          surface: "background",
    ...
    async handleGetAuthStatus(sendResponse) {
      try {
        ...
        console.error("Failed to get auth status:", error2);
    ...
    async handleGetApiDomain(sendResponse) {
      try {
        ...
        console.error("Failed to get API domain:", error2);
    ...
    async handleApiRequest(message, sendResponse) {
      try {
        ...
        if (!response.ok) {
          ...
          this.enqueueTelemetry({
            event_type: response.status === 429 ? "rate_limited" : response.status === 402 ? "credits_exhausted" : "api_request_failed",
            surface: "background",
            route: endpoint,
        ...
        if (this.debug) {
          console.error("Background API request failed:", error2);
        }
        this.enqueueTelemetry({
          event_type: String(error2?.message || "").toLowerCase().includes("timeout") ? "api_timeout" : "api_request_failed",
          surface: "background",
          route: message?.endpoint || "",
          error_code: error2?.message || "api_request_exception"
        });
        const errObj = error2 instanceof Error ? error2 : new Error(String(error2?.message ?? error2));
    ...
    async openWelcomePage() {
      try {
        ...
        console.error("Failed to open welcome page:", error2);
      }
    }
    handleOpenLoginPage(url, sendResponse) {
      try {
        ...
        console.error("Failed to open login page:", error2);
